...
class CountryCodes
{
    ...
    /**
     * @return array<int, array{name: string, dial_code: string}>
     */
    public static function all(): array
    {
        ...
        $ordered = array_merge(
            self::PRIORITY,
            array_values(array_diff(array_keys($codes), self::PRIORITY))
        );

        return array_map(
            fn (string $name) => ['name' => $name, 'dial_code' => $codes[$name]],
            $ordered
        );
    }

    public static function dialCodeFor(string $countryName): ?string
    {
        return self::codes()[$countryName] ?? null;
    }
    ...
    private static function codes(): array
    {
        return [
            ...
            'Israel' => '+972', 'Italy' => '+39', 'Jamaica' => '+1876', 'Japan' => '+81', 'Jordan' => '+962',
